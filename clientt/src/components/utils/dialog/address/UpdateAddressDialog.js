...
function UpdateAddressDialog({ _id, openDialog, setOpenDialog, classes, getAddressList }) {
  const {updateAddressApi} = AddressJwtApi()
  ...
  const handleSubmit = async () => {
    try {
      const updateAddresss = await updateAddressApi(_id, location)
      getAddressList()
      setMessage(updateAddresss.message);
      setOpenSnack(true)
  ...
  const changeProvince = async (e) => {
    ...
    getDistricts(provinceData)
  }

  const getDistricts = async (provinceData) => {
    const districtRequest = await getDistrictsApi(provinceData.code)
    setDistricts(districtRequest)
  }
  ...
  const getWards = async (districtData) => {
  ...
  return (
    <>
      ...
      >
        <DialogTitle id="form-dialog-title">Cập nhập địa chỉ</DialogTitle>
        <DialogContent>
